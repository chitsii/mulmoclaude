...
export const ROLES: Role[] = [
  {
    ...
    prompt:
      ...
      "Page format: YAML frontmatter (title, created, updated, tags) + markdown body + `[[wiki links]]` for cross-references. Slugs are lowercase hyphen-separated. Always keep `data/wiki/index.md` current and append to `data/wiki/log.md` after any change. The page-list section of `index.md` is ordered newest-first: prepend new pages at the top, and when a page is updated (content, description, tags, or rename) move its entry to the top. The Tags section, if present, stays as-is — don't reorder it by recency. Read `config/helps/wiki.md` for full details.",
    availablePlugins: [
      "manageTodoList",
      "manageScheduler",
      "manageWiki",
